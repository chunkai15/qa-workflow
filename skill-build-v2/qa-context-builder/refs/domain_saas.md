...
> Load immediately when qa_context_builder starts. Used to identify roles, entities, and risks specific to SaaS.
...
### High-Risk Areas (always check when analyzing spec)
- **Credit system:** issuance → consumption → balance tracking. Errors here = financial errors
- **Multi-tenant isolation:** Coach A must not see Coach B's data (strict isolation)
- **Permission/RBAC:** each role sees and does different things — check every action per role
...
## Common Cross-Domain Patterns (always check)
