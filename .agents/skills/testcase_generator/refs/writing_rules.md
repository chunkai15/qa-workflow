...
Sequence resets to `001` cho mỗi US+category combination mới.
Nếu spec không có explicit US IDs → đánh số sequential: US-01, US-02...
...
- `[STEP_03] EDGE | Neg | Risk: timezone boundary` — edge case với risk tag
- `[MOD_BIZ] SEC | Neg | Auth behavior` — security TC
- `[MOD_01] SCEN | E2E | Assume: A1` — scenario TC với assumption flag
...
**H (High):** Primary revenue/user-critical workflows | Security/permission risks | Mandatory validations | Data loss or irreversible operations | Billing/payment flows

**M (Medium):** Common alternate paths | Common error handling | Recoverable failures | Standard UI feedback

**L (Low):** Lower-frequency cosmetic behavior | Unlikely edge cases | Optional secondary flows | Non-critical UI details
